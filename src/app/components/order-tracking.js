...
export default function OrderTrack({order}) {
  ...
  const isButtonDisabled = order.status === OrderStatus.CANCELLED
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-sm font-medium ${getStatusColor(order.status)}`}>Status: {order.status}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-sm font-medium text-gray-900">{formatterCurrency.format(order.product.price)}</p>
          ...
          <div key={step.id} className="flex items-start gap-4 relative">
            <div className="flex-shrink-0 mt-1 relative z-10">
              {step.completed ? (
                ...
                <div className={`w-6 h-6 rounded-full ${step.cancelled ? "bg-red-500" : "bg-orange-500"}`}>
                  {step.cancelled && (
